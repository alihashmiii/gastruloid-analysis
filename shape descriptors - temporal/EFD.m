...
harmonicAnalysis::usage = "harmonicAnalysis[p_,q_,r_,s_] takes in four Elliptical Fourier Coefficients and outputs the Length, Width and Orientation of the Ellipse";
EllipticalFourier::usage = "EllipticalFourier[img_Image,opts] takes in a binarized mask representing the object's shape and outputs an array of Fourier Elliptical
Coefficients that describe the shape. A 2 % difference approximation is used by default. Use locus -> \"dcCoefficients\" ";


Begin["`Private`"]


harmonicAnalysis[p_,q_,r_,s_]:=Block[{coeff,\[Theta],rotationMat,starMat,\[Psi],L,W,\[Phi]},
...
EllipticalFourier[img_Image,OptionsPattern[]]:=Module[{dXY,dt,t,T,\[Phi],contourpositions,contour,contourtour,contourregion,xi,A0,\[Delta],C0,
coeffL,normalizeEFD,ellipticalCoefficients,plotEFD,dcCoefficients,dcVal},
...
nhalf = Ceiling[n/2];
ti = Array[#&,i,{0.,1}];
dcVal = If[OptionValue[locus]==="dcCoefficients",dcCoefficients, OptionValue@locus];
...
{Thick,Dashed,XYZColor[1,0,0,0.6],Line@contour},
Inset[Style[#,{Black,FontSize->12}]&@Text["harmonic: " <> ToString@j]]}]};
j++];
];
